...
var parseScript = function(id, url, engine) {
   ...
   pc2d.script.createLoadingScreen = function() {
        __results.loading = true;
    };

    // implementpc2d.createScript
   pc2d.createScript = function(name, app, script) {
        var valid = true;
        ...
        if (script) {
            ...
            } else if (!(script.prototype instanceof pc2d.ScriptType)) {
                if (!name) {
                    name = 'script';
                }
                valid = false;
                __results.scriptsInvalid.push('script class: \'' + name + '\' does not extendpc2d.ScriptType.');
            }
        }
        ...
        if (script && script.attributes) {
            // overridepc2d.ScriptType.attributes.add
    ...
    // implementpc2d.registerScript
   pc2d.registerScript = function (script, name, app) {
        if (typeof script === 'function' && script.prototype instanceof pc2d.ScriptType) {
            name = name || script.__name ||pc2d.ScriptType.__getScriptName(script);
        }

        returnpc2d.createScript(name, app, script);
    };

    // override ScriptAttributes#add to throw an error
    // which will happen if users try to call this method before calling
    // registerScript
   pc2d.ScriptAttributes.prototype.add = function (name) {
        __results.scriptsInvalid.push('script `' + this.scriptType.name + '` you have to callpc2d.registerScript orpc2d.createScript before declaring script attributes.');
    };
