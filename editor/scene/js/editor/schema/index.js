...
editor.once('load', function() {
    ...
    if (schema.element) {
        schema.element.fontAsset.$default = function () {
            ...
            return firstAvailableFont ? firstAvailableFont[1].get('id') : null;
        };
    }
...
editor.method('components:convertValue', function (component, property, value) {
    ...
    if (value) {
        ...
        if (data && data[property]) {
            ...
            switch (type) {
                case 'rgb':
                    result = new pc2d.Color(value[0], value[1], value[2]);
                    break;
                case 'rgba':
                    result = new pc2d.Color(value[0], value[1], value[2], value[3]);
                    break;
                case 'vec2':
                    result = new pc2d.Vec2(value[0], value[1]);
                    break;
                case 'vec3':
                    result = new pc2d.Vec3(value[0], value[1], value[2]);
                    break;
                case 'vec4':
                    result = new pc2d.Vec4(value[0], value[1], value[2], value[3]);
                    break;
                case 'curveset':
                    result = new pc2d.CurveSet(value.keys);
                    result.type = value.type;
                    break;
                case 'curve':
                    result = new pc2d.Curve(value.keys);
                    result.type = value.type;
                    break;
                case 'entity':
