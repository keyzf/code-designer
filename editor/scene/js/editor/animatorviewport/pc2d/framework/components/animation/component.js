Object.assign(pc2d, function () {
    ...
    AnimationComponent.prototype = Object.create(pc2d.Component.prototype);
    ...
    Object.assign(AnimationComponent.prototype, {
        ...
        play: function (name, blendTime) {
            ...
            if (!data.animations[name]) {
                // #ifdef DEBUG
                console.error(pc2d.string.format("Trying to play animation '{0}' which doesn't exist", name));
                // #endif
                return;
            }
            ...
            if (data.model) {
                ...
                if (data.animController) {
                    ...
                    var clip = new pc2d.AnimClip(data.animations[data.currAnim], 0, 1.0, true, data.loop);
        ...
        _createAnimationController: function () {
            ...
            for (var animation in animations) {
                if (animations.hasOwnProperty(animation)) {
                    ...
                    if (anim.constructor === pc2d.AnimTrack) {
                        hasGlb = true;
                    } else {
                        hasJson = true;
                    }
            ...
            if (hasJson) {
                data.fromSkel = new pc2d.Skeleton(graph);
                data.toSkel = new pc2d.Skeleton(graph);
                data.skeleton = new pc2d.Skeleton(graph);
                data.skeleton.looping = data.loop;
                data.skeleton.setGraph(graph);
            } else if (hasGlb) {
                data.animController = new pc2d.AnimController(new pc2d.DefaultAnimBinder(graph));
            }
        },
        ...
        onSetAssets: function (name, oldValue, newValue) {
            ...
            var ids = newValue.map(function (value) {
                return (value instanceof pc2d.Asset) ? value.id : value;
            });
        ...
        onEnable: function () {
            pc2d.Component.prototype.onEnable.call(this);
            ...
            if (assets) {
                for (var i = 0, len = assets.length; i < len; i++) {
                    var asset = assets[i];
                    if (!(asset instanceof pc2d.Asset))
