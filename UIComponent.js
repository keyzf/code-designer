;(function(window){
    ...
    class UIComponent extends HTMLElement {
        ...
        connectedCallback(){     //attachedCallback
            ...
            for (var i = 0, l = attributes.length; i < l; i++) {
                ...
                if ((attrNameMatch = EVENT_ATTRIBUTE_RE.exec(attribute.name))) {
                    ...
                    if (typeof handlerFunc === "function") {
                        ...
                        console.warn(this.nodeName + ": event handler \"" + handlerName + "\" is not " + "a function");
                    }
                }
        ...
        compile(nodeItem){
            var attributes = ['click','tap'].map(attr => ({
                name: "bind:" + attr,
                value: nodeItem.getAttribute( "bind:" + attr)
            })).filter(attr => attr.value);

            var attributes2 = ['for','for-items','key'].map(attr => ({
                name: "wx:" + attr,
                value: nodeItem.getAttribute( "wx:" + attr)
            })).filter(attr => attr.value);



            var EVENT_ATTRIBUTE_RE = /^bind:(.+)$/ ;
            for (var i = 0, l = attributes.length; i < l; i++) {
                var attribute = attributes[i],
                    attrNameMatch;
                if ((attrNameMatch = EVENT_ATTRIBUTE_RE.exec(attribute.name))) {
                    ...
                    if (typeof handlerFunc === "function") {
                        ...
                        if(eventName === "tap"){
                            var mc = new Hammer.Manager(nodeItem, {});
                            mc.add( new Hammer.Tap({ event: eventName, taps: 1 }) );
                           
                            mc.on(eventName, handlerFunc);
                            nodeItem['mc'] = mc;

                            //this.listenTo(nodeItem,eventName,handlerFunc)
                        } else{
                            this.listenTo(nodeItem,eventName,handlerFunc)   
                        }                          
                    } else {
                        console.warn(nodeItem.nodeName + ": event handler \"" + handlerName + "\" is not " +  handlerName ? "a function" : "defined");
                    }
                }
            } 
            
            var DIRECTIVE_ATTRIBUTE_RE = /^wx:(.+)$/ ;
            var forAttribute = attributes2[0],attrNameMatch;

            /**TODO */
            if(forAttribute){
                if ((attrNameMatch = DIRECTIVE_ATTRIBUTE_RE.exec(attribute.name))) {
                    var eventName = attrNameMatch[1],
                        foritem = attributes2[1],
                        handlerName = attribute.value,
                        _data = this.data;
                        
                        var pattern = /^\{\{([^\{\}]*)\}\}$/g;
                        var b = pattern.exec(handlerName);
                        var p = b[1].split('.'),
                            r = _data;
                          try {
                            for (var s in p) {
            ...
            var nodeList = nodeItem.childNodes;
