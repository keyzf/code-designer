...
importScripts("kvStorage.js", "localForageAdaptor.js");

const OFFLINE_DATA_FILE = "offline-root.json";
const CACHE_NAME_PREFIX = "c3-root";
const BROADCASTCHANNEL_NAME = "root-offline";
const CONSOLE_PREFIX = "[Root-SW-Offline] ";
const LAZYLOAD_KEYNAME = "c3-root-sw-lazyload";

// Optional additional cache cleanup when upgrading.
// Make sure the caches used by specific editor versions are also cleaned up.
const ALSO_CLEANUP_CACHE_NAME_PREFIX = "c3-https://editor.construct.net/";
const ALSO_CLEANUP_KEEP_COUNT = 3;
...
// Since the caches are stored in 
...
async function HandleFetch(event, doUpdateCheck)
{
	...
	const result = await Promise.all([fetch(request), ReadLazyLoadListFromStorage()]);
	const fetchResponse = result[0];
	const lazyLoadList = result[1];
	
	// NOTE: don't cache partial responses (status code 206) to lazy-cached URLs.
	if (IsUrlInLazyLoadList(request.url, lazyLoadList) && fetchResponse.status !== 206)
	{
		...
		{
			console.warn(CONSOLE_PREFIX + "Error caching '" + request.url + "': ", err);
		}
	}
...
async function HandleBetaRedirect(event)
{
	try {
		// Attempt the request
	...
	{
		// If the fetch fails, try to load the last used beta release and redirect to that.
		// If there isn't anything stored, rethrow the error so the redirect fails as per the original fetch.
		const lastUsedBeta = await localforage.getItem("c3-last-beta-release");
		if (typeof lastUsedBeta === "string")
			return Response.redirect(location.origin + "/" + lastUsedBeta + "/");
		else
			throw err;
	}
}
...
self.addEventListener("fetch", event =>
{
	...
	// If the URL is editor.construct.net/beta specifically, handle as a redirect
	// to the last used beta release.
	if (event.request.url === location.origin + "/beta")
	{
		event.respondWith(HandleBetaRedirect(event));
		return;
	}
